...
<h2 class="text-white font-black uppercase tracking-wider text-2xl mb-6">Editar Serviço</h2>

<form action="{{ route('admin.services.update', $service) }}" method="POST" class="max-w-2xl">
    @csrf
    @method('PUT')
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-6">
        <x-forms.input name="name" label="Nome do serviço" :value="$service->name" required />
